...
[![Version](https://img.shields.io/badge/version-1.11.3-8b5cf6?style=flat-square)](releases/knowledge-atlas-1.11.3.zip)
...
1. 下载 [`knowledge-atlas-1.11.3.zip`](releases/knowledge-atlas-1.11.3.zip)。
...
创建 `<你的-vault>/.obsidian/plugins/knowledge-atlas/`，然后复制以下生成文件与运行文件：
...
- 或在命令面板运行 **Knowledge Atlas: Open atlas**。
- 点击行星或卫星进入对应目录。
...
可读源码位于 `src/main.js`。生产版本通过 esbuild 构建，编译后的 `main.js` 仅作为 GitHub Release 附件发布，不在仓库中跟踪。
...
4. 使用 `node --check main.js` 检查生成文件语法。
5. 在 Obsidian 中实际测试，并检查开发者错误。
6. 创建与 Manifest 版本完全一致的 GitHub Release tag。
7. 将 `main.js`、`manifest.json` 和 `styles.css` 添加到 Release 附件。
